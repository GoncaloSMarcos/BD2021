...
@app.route("/empregados/", methods=['GET'], strict_slashes=True)
def get_all_empregados():
    logger.info("###              DEMO: GET /empregados              ###");   

    conn = db_connection()
    cur = conn.cursor()

    cur.execute("SELECT nome, nemp FROM emp")
    rows = cur.fetchall()

    payload = []
    logger.debug("---- empregados  ----")
    for row in rows:
        logger.debug(row)
        content = {'nome': row[0], 'nemp': int(row[1])}
        payload.append(content) # appending to the payload to be returned
...
def get_department(ndep):
    ...
    try:
        ...
        return jsonify('ERROR: Department missing from database!')
